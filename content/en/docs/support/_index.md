---
title: Support
description: "Podstack GPU cloud support - troubleshooting guides, documentation, and help resources for GPU containers, VMs, storage, and billing issues."
keywords:
  - GPU cloud support
  - GPU troubleshooting
  - cloud computing help
  - ML infrastructure support
---

# Support

Get help with Podstack through our support resources and troubleshooting guides.

## Getting Help

### Documentation

Browse this documentation for:
- Feature guides
- Step-by-step tutorials
- Best practices
- Troubleshooting tips

### Contact Support

For issues not covered in documentation:
- Email: support@podstack.ai
- Response time: Within 24 hours

When contacting support, include:
- Your account email
- Resource IDs (pod, VM, etc.)
- Error messages or screenshots
- Steps to reproduce the issue

## Common Topics

### Account Issues
- [Creating an account](/docs/getting-started/creating-account/)
- [Login problems](/docs/support/troubleshooting/#login-issues)
- [Account settings](/docs/account/settings/)

### Billing Questions
- [Wallet management](/docs/billing/wallet/)
- [Invoice queries](/docs/billing/invoices/)
- [Payment issues](/docs/support/troubleshooting/#payment-issues)

### Resource Problems
- [Pod troubleshooting](/docs/support/troubleshooting/#pod-issues)
- [VM troubleshooting](/docs/support/troubleshooting/#vm-issues)
- [Storage issues](/docs/support/troubleshooting/#storage-issues)

### Connection Issues
- [SSH access](/docs/compute/pods/connecting-to-pods/)
- [Network problems](/docs/support/troubleshooting/#network-issues)

## Status Page

Check platform status for ongoing incidents or maintenance:
- Service availability
- Scheduled maintenance
- Incident reports

## Feedback

Help us improve Podstack:
- Report bugs
- Suggest features
- Share your experience

## In This Section

- **[Troubleshooting](/docs/support/troubleshooting/)** - Solutions to common problems
